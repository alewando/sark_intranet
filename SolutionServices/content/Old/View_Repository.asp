...
'Session("Username")="bsamson"
'Session("Username")="dpodnar"
'Session("Username")=Application("WebMaster")
'Session("isSolutionServices")=null
...
'Response.Write "isSolutionServices=" & Session("isSolutionServices") & "<br>"
...
If Request("ACTION") = ACTION_DELETE Then
	...
	If Request.Form("DELETE_FILE").Count And (Lcase(Session("username")) = Lcase(Application("WebMaster")) Or canMaintainRepos) Then
		'----------------------------------------------------------------------------
	...
	If Request.Form("DELETE_FOLDER").Count And Lcase(Session("username")) = Lcase(Application("WebMaster")) Then
	   x = 1
...
If Request("NEWFOLDER") & "x" <> "x" Then
	...
	fso.CreateFolder(server.mappath(relativePath & "/" & Request("NEWFOLDER")))
End If
...
<TR>
	<TD BGCOLOR=#FFFFFF ALIGN=LEFT COLSPAN=3>
		<FONT SIZE=1 FACE="ms sans serif, arial, geneva" COLOR=black>
			...
			<%If Lcase(Session("username")) = Lcase(Application("WebMaster")) Or canMaintainRepos = True Then
			'-------------------------------------------------------------------------------
...
if collFolders.count <> 0  then
	...
	For Each whatever in collFolders
		...
		if Lcase(Session("username")) = Lcase(Application("WebMaster")) Then
...
<tr><td colspan=3>
	<table border=0 width=100% ><%							if collFiles.count <> 0  then
		...
		For Each whatever in collFiles
			...
			response.write server.URLEncode(relativePath & "/" & whatever.name)
...
function openWindow(URL) 
{
	...
	{
		winUploadDoc=window.open(URL, "OpenDocument","height=350,width=580,toolbar=0,location=0,directories=0,status=0,menubar=0,resizable=0,scrollbar=1");
