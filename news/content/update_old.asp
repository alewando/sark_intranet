...
<%
	...
	'-------------------------------------------------------------------------
	filename = Application("WebRootDir") & Application("web") &"\news\content\articles\article" & UpdateArticleNumber & ".txt"
	'set fso = Server.CreateObject("Scripting.FileSystemObject")
	...
	If Request.Form.Item("Title") <> "" then 
		If Request.Form("DelFlag") <> "True" then
			...
			'-------------------------------------------------------------------------
			If Ucase(Session("Username")) = ucase(Application("WebMaster")) then
		...
		if ApproveStat = False then
			Response.Write("<INPUT type=button class=button value='Cancel' onClick='window.location=" & chr(34) & "../../tools/content/approve_postings.asp" & chr(34) & "' id=button name=button></CENTER>")
		else
			Response.Write("<INPUT type=button class=button value='Cancel' onClick='window.location=" & chr(34) & "about.asp" & chr(34) & "' id=button name=button></CENTER>")	
		end if
