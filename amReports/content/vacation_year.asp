...
Date:         08/16/2000
Description:  Report of vacation in the next 12 months by month
...
<html>
<head><title>Vacation Scheduled for the Next 12 Months</title></head>

<body>
<%
sql = "SELECT v.vacation_start, v.vacation_end, e.firstname, e.lastname, e.employeeID " &_
		"FROM vacation v, employee e WHERE datediff(mm, GETDATE(), vacation_start) > 0 " &_
		"AND datediff(mm, GETDATE(), vacation_start) < 13 AND v.employee_id = e.employeeid " &_
		"ORDER BY e.lastname, e.firstname, vacation_start" 
set rsVacation = DBQuery(sql)
%>

<font size=3 face='ms sans serif, arial, geneva' color=black>
<b>Vacation Scheduled for the Next 12 Months</b>
<p>
