...
<html>
<head><title>Vacations in the past two weeks</title></head>
<body>
 
<%
sql = "SELECT v.vacation_start, v.vacation_end, e.firstname, e.lastname, e.employeeID " &_
		"FROM vacation v, employee e WHERE v.employee_id = e.employeeid AND ((datediff(dd, vacation_start, GETDATE()) > 0 " &_
		"AND datediff(dd, vacation_start, GETDATE()) < 14) or (datediff(dd, vacation_end, GETDATE()) > 0 " &_
		"AND datediff(dd, vacation_end, GETDATE()) < 14)) " &_
...
<b>Vacations in the past two weeks</b>
<p>
