...
Dim intSkillID
Dim intSkillRanking
Dim intOtherSkillValue
Dim arySkillIDs()
Dim arySkillRankings()
Dim aryOtherSkillValue()
Dim j
intSkillID = 0
intSkillRanking = 0
intOtherSkillValue = 0
...
'Populate an array with the skill rankings for each skill.
...
'Populate an array with the skill value for each skill.
For Each item in Request.Form("OtherSkill")
  ReDim Preserve aryOtherSkillValue(intOtherSkillValue + 1)
  intOtherSkillValue = intOtherSkillValue + 1
  aryOtherSkillValue(intOtherSkillValue) = Request.Form("OtherSkill")(intOtherSkillValue)
Next
...
IF rs.eof THEN
	...
	For j = 1 to UBound(arySkillIDs)
		...
		OtherSkillValue = aryOtherSkillValue(j)
		sql = "Insert into Skills_Inventory (EmployeeID, SkillRanking, SkillID, SkillValue) " &_
		  "Values ('" & empID & "','" & SkillRanking & "' , '" & SkillID & "' , '" & OtherSkillValue & "')"
		DBQuery(sql)
	Next
Else 
	'-------------------------------------------
