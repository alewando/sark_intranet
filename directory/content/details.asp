...
{
		winUpdate=window.open(URL + "?empID=" + EmpID, WinTitle,"height=615,width=575,toolbar=0,location=0,directories=0,status=0,menubar=0,resizable=1,scrollbars=1")
...
<!--Code added here by Ryan Dlugosz on 6/20/00 to implement tabbed information interface
	...
	<TABLE ID="idTabs" CELLPADDING="2" CELLSPACING="0" BORDER="0" BGCOLOR="#ffffcc" STYLE="color: #FFFFFF; display:none; behavior: url(#default#savehistory);" ONSAVE="fnSaveState()" ONLOAD="fnGetState()">
		<TR HEIGHT="25" VALIGN="middle">
			<TD ID="tabs" CLASS="clsTabSelected" ONCLICK="fnTabClick(profileTabNum);">&nbsp;<A HREF="javscript:void();" ONCLICK="return fnTabClick(profileTabNum);">Profile</A>&nbsp;</TD>
			<TD ID="tabs" CLASS="clsTab" ONCLICK="fnTabClick(technologyTabNum);">&nbsp;<A HREF="javascript:void();" ONCLICK="return fnTabClick(technologyTabNum);">Technologies</A>&nbsp;</TD>
			<TD ID="tabs" CLASS="clsTab" ONCLICK="fnTabClick(certsTabNum);">&nbsp;<A HREF="javascript:void();" ONCLICK="return fnTabClick(certsTabNum);">Certifications</A>&nbsp;</TD>
			<TD ID="tabs" CLASS="clsTab" ONCLICK="fnTabClick(industryTabNum);">&nbsp;<A HREF="javascript:void();" ONCLICK="return fnTabClick(industryTabNum);">Industry&nbsp;Experience</A>&nbsp;</TD>
			<TD ID="tabs" CLASS="clsTab" ONCLICK="fnTabClick(projectsTabNum);">&nbsp;<A HREF="javascript:void();" ONCLICK="return fnTabClick(projectsTabNum);">Projects</A>&nbsp;</TD>
			<% if hasRole("AccountManager") or hasRole("Sales") then %>
			<TD ID="tabs" CLASS="clsTab" ONCLICK="fnTabClick(vacationTabNum);">&nbsp;<A HREF="javascript:void();" ONCLICK="return fnTabClick(vacationTabNum);">Vac</A>&nbsp;</TD>
			<% end if %>
...
-->
			...
			<TD ID="tabs" CLASS="clsTab" ONCLICK="fnTabClick(reviewsTabNum);">&nbsp;<A HREF="javascript:void();" ONCLICK="return fnTabClick(reviewsTabNum);">Rev</A>&nbsp;</TD>
			<% end if %>
			<% if hasRole("AccountManager") or hasRole("Sales") then %>
			<TD ID="tabs" CLASS="clsTab" ONCLICK="fnTabClick(examsTabNum);">&nbsp;<A HREF="javascript:void();" ONCLICK="return fnTabClick(examsTabNum);">Exam</A>&nbsp;</TD>
			<TD ID="tabs" CLASS="clsTab" ONCLICK="fnTabClick(classesTabNum);">&nbsp;<A HREF="javascript:void();" ONCLICK="return fnTabClick(classesTabNum);">Classes</A>&nbsp;</TD>
			<% end if %>
		</TR>		
	</TABLE>
...
<SCRIPT>newsContent[technologyTabNum].style.display="none";var sText=1</SCRIPT>

 <SCRIPT Language="Javascript">
  certsTabNum=maxTabNum++;
 </SCRIPT>
 <A NAME="Certs"></a>
...
if (webmasterButtons) then
	'Display cert #'s (if any)
	sql = "SELECT cert_number FROM employee_cert_numbers WHERE employee_id = " & ls_empid & " AND category_id=1"
	set rs = DBQuery(sql)
	if not rs.eof and not IsNull(rs("cert_number")) then
		Response.Write("<B>MCP #: " & rs("cert_number") & "</b><BR>")
	end if
	...
	'----------------------------------------------------------------
		Response.Write("<form><INPUT TYPE=button class=button NAME='Certifictaions' " & _
		               ...
		               "openWindow('certifications.asp', " & ls_empid  & ", 'EditSARKCert')" & chr(34) & "></form>")
end if             
...
<SCRIPT>newsContent[certsTabNum].style.display="none";var sText=1</SCRIPT>

 <SCRIPT Language="Javascript">
  industryTabNum=maxTabNum++;
 </SCRIPT>
 <A NAME="Industry"></a>
...
<SCRIPT>newsContent[industryTabNum].style.display="none";var sText=1</SCRIPT>

 <SCRIPT Language="Javascript">
  projectsTabNum=maxTabNum++;
 </SCRIPT> 
 <A NAME="Projects"></a>
...
<SCRIPT>newsContent[projectsTabNum].style.display="none";var sText=1</SCRIPT>
...
<SCRIPT>	
 newsContent[reviewsTabNum].style.display="none";
 var sText=1
</SCRIPT>
...
Response.write("<td width=90 align=left><font size=1 face='ms sans serif, arial, geneva'>")
Response.Write("<b>Vacation ID:</b>")
Response.write("<td width=70 align=left><font size=1 face='ms sans serif, arial, geneva'>")
Response.Write("<b>Start: </b>")
Response.write("<td width=70 align=left><font size=1 face='ms sans serif, arial, geneva'>")
Response.Write("<b>End: </b>")
Response.write("<td width=150 align=left><font size=1 face='ms sans serif, arial, geneva'>")
Response.Write("<b>Comments: </b>")
Response.write("</tr></table>")
end if
count=1
while not rs.eof 
	ls_empid = trim(rs("employee_id"))
	...
	if Remainder <> 0 then
		Response.write("<table bgcolor=white><td><font size=1 face='ms sans serif, arial, geneva'>")
	else
		Response.write("<table><td><font size=1 face='ms sans serif, arial, geneva'>")
	end if
	if not (isnull(ls_vacid) or trim(ls_vacid) = "") then
		Response.Write("<td width=90 align=left><b><a href=""javascript:addDocument(" & ls_vacid & ");"" onMouseOver=""top.status='Update Vacation'; return true;"" onMouseOut=""top.status=''; return true;"">" & ls_vacid & "</a>&nbsp;&nbsp;&nbsp;")
		end if
	if not (isnull(ls_vacstart) or trim(ls_vacstart) = "" ) then
		Response.Write("<td width=70 align=left><b>" & "</b>" & ls_vacstart)
		end if
	if not (isnull(ls_vacend) or trim(ls_vacend) = "") then
		Response.Write("<td width=70 align=left><b>" & "</b>" & ls_vacend)
		end if
	if not (isnull(ls_vaccomments) or trim(ls_vaccomments) = "") then
		Response.Write("<td width=150 align=left><b>" & "</b>" & ls_vaccomments & "<br><br>")
		end if
...
rs.close
set rs = nothing
 'Only Account Managers get the buttons
 if hasRole("WebMaster") or hasRole("AccountManager") then
...
<SCRIPT>newsContent[vacationTabNum].style.display="none";var sText=1</SCRIPT>	


<% end if %>  <!-- end for the security feature (vacation tab)-->
...
ls_sql = "SELECT Exam_List.Exam_ID, Exam_List.Exam_Name, Exam_list.cert," _
...
while not rs.eof 
	...
	Remainder=count mod 2
	if Remainder <> 0 then
		Response.write("<table bgcolor=white><td><font size=1 border=3 face='ms sans serif, arial, geneva'>")
	else
	...
	if not (isnull(ls_examname) or trim(ls_examname) = "" ) then
		...
		Response.Write("<td colspan=1 width=250 valign=middle align=left>")
		Response.Write("<strong> "& ls_examname &" </strong>")
		else
...
Response.Write("<center><b>* Bold items represent certifications</b></center>")
...
<SCRIPT>newsContent[examsTabNum].style.display="none";var sText=1</SCRIPT>

<% end if %> 

<A NAME="Classes"></a>
	<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%" ID="tblHeadClasses">
	<TR>
	<TD ALIGN="left" WIDTH="100%" HEIGHT="25" COLSPAN="2"><IMG HEIGHT="25" src="ts.gif" WIDTH="15"></TD>
	</TR>
	<TR> 
	<TD VALIGN="middle" HEIGHT="18" BGCOLOR="#ffffcc" NOWRAP><IMG HEIGHT="18" src="ts.gif" WIDTH="1">&nbsp;&nbsp;<font COLOR="#000000" SIZE="2"><B>Classes</B></FONT>&nbsp;&nbsp;</TD>
	<TD ALIGN="right" WIDTH="100%"></TD>
	</TR>
	<TR>
	<TD ALIGN="left" BGCOLOR="#ffffcc" VALIGN="top" WIDTH="100%" HEIGHT="6" COLSPAN="2"><IMG SRC="ts.gif" ALT HEIGHT="6" WIDTH="2"></TD>
...
<SCRIPT>tblHeadClasses.style.display="none";</SCRIPT>
...
<% 	'--------------------------------
	'Execute query for Classes
	'--------------------------------
						
ls_sql = "SELECT Location.Location_ID, Location.Location_Name, Class_List.Class_ID, Class_List.Class_Name," _
	& " Classes.Class_taken_ID, Classes.Class_ID, Classes.Class_Start_Date, Classes.Class_End_Date," _
	& " Classes.Comments" _
	& " FROM Location, Class_List   INNER JOIN" _
	& " Classes ON" _
    & " Classes.Class_ID = Class_List.Class_ID" _
	& " WHERE (Classes.EmployeeID = " & ls_empid & ")" _
	& " AND Location.Location_ID = Classes.Location_ID" _
	& " ORDER BY employeeID, Classes.Class_Start_Date"
...
count=1
while not rs.eof 
	ls_classid = rs("Class_taken_ID")
	ls_classname = trim(rs("Class_Name"))
	ls_classStartdate = trim(rs("Class_Start_Date"))
	ls_classEnddate = trim(rs("Class_End_Date"))
	ls_location = trim(rs("Location_Name"))
	ls_comments = trim(rs("Comments"))
	Remainder = count mod 2
	if Remainder <> 0 then
		Response.write("<table border=0 bgcolor=white width=425><td><font size=1 border=1 face='ms sans serif, arial, geneva'>")
	else
		Response.write("<table border=0 width=425><td><font size=1 border=1 face='ms sans serif, arial, geneva'>")
	end if
	if not (isnull(ls_classid) or trim(ls_classid) = "") then
	    'Response.write("<td  colspan=2 width=60 valign=middle align=left><font size=1 face='ms sans serif, arial, geneva'>")
        'Response.Write("<b>Class ID:</b>")
	    'Response.write("<td  width=20 valign=middle align=left><font size=1 face='ms sans serif, arial, geneva'>")
		Response.Write("<td colspan=2 width=20 align=left><b><a href=""javascript:addDocument3(" & ls_classid & ");"" onMouseOver=""top.status='Update Class'; return true;"" onMouseOut=""top.status=''; return true;"">" & ls_classid & "</a></b>")
		end if
	if not (isnull(ls_classname) or trim(ls_classname) = "" ) then
	    Response.write("</td><td width=30 align=left><font size=1 face='ms sans serif, arial, geneva'>")
        Response.Write("<b>  Name:</b></td>")
		Response.Write("<td width=175 align=left>" & ls_classname)
		Response.write("</td><td width=50 align=left><font size=1 face='ms sans serif, arial, geneva'>")
		Response.Write("<b>Comments:</b>")
		Response.Write("</td><td width=175 align=left>" & ls_comments)
		Response.write("</td></tr></table>")
		if Remainder <> 0 then
			Response.write("<table border=0 width=425 bgcolor=white><tr><font size=1 face='ms sans serif, arial, geneva'>")
		else
			Response.write("<table border=0 width=425><tr><font size=1 face='ms sans serif, arial, geneva'>")
		end if
		end if
	if not (isnull(ls_location) or trim(ls_location) = "") then
		Response.write("<td width=50 align=left><font size=1 face='ms sans serif, arial, geneva'>")
		Response.write("</td><td width=10 align=left><font size=1 face='ms sans serif, arial, geneva'>")
        Response.Write("<b>City:</b></td>")
		Response.Write("<td width=50 align=left>" & ls_location & "</td>")
		end if
	if not (isnull(ls_classStartdate) or trim(ls_classStartdate) = "") then
     	Response.write("<td width=10 align=left><font size=1 face='ms sans serif, arial, geneva'>")
        Response.Write("<b>Date:</b></td>")
		Response.Write("<td width=110 align=left>" & ls_classStartdate & " - " & ls_classEndDate & "</td>")
		end if
	'if not (isnull(ls_comments) or trim(ls_comments) = "") then
	'	Response.Write("<td colspan=1 width=150 valign=middle align=left><b>" & "</b>"& ls_comments)
	'	end if
Response.write("</td></tr></table>")
	count=count+1
	rs.movenext
...
'Only account managers get the buttons
if hasRole("WebMaster") or hasRole("AccountManager") then
 Response.Write("<center><table><center><td><form id=form10 name=form10> " &_
 "<INPUT TYPE=button class=button NAME='Add Class' " &_
	               "VALUE = '  Add Class  ' onClick=" & chr(34) & _
	               "openWindow('addclass.asp', " & ls_empid & ", 'EditSarkClass')" & chr(34) & "> " &_
 "<INPUT TYPE=hidden NAME='tabID' VALUE = '7'> " &_
				   "</form></td></center></table></center>")
end if
%>
...
<SCRIPT>newsContent[classesTabNum].style.display="none";var sText=1</SCRIPT>
...
{
	winUpdate=window.open("updateVacation.asp?vacid=" + arg, "AddDocument","height=615,width=500,toolbar = 0,location=0,directories=0,status=0,menubar=0,resizable=1,scrollbar=1")
	intervalID=setInterval('checkChildWindow()', 100)
	state = fnSaveState();
...
{
	winUpdate=window.open("updateexam.asp?epid=" + arg, "AddDocument","height=615,width=500,toolbar = 0,location=0,directories=0,status=0,menubar=0,resizable=1,scrollbar=1")
	intervalID=setInterval('checkChildWindow()', 100)
	state = fnSaveState();

}

function addDocument3(arg) 
{
	winUpdate=window.open("updateclass.asp?ctid=" + arg, "AddDocument","height=615,width=500,toolbar = 0,location=0,directories=0,status=0,menubar=0,resizable=1,scrollbar=1")
	intervalID=setInterval('checkChildWindow()', 100)
	state = fnSaveState();
