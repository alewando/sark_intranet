...
ElseIf Request("InsertUpdate") = "CERT" Then
	...
	For Each ls_certType In Request("CertNum")
		ls_CertNum = Request("CertNum"& ls_certType & "Value")
		'Remove old value (if any)
		ls_sql = "DELETE FROM employee_cert_numbers WHERE employee_id = " & ls_empid & " AND category_id = " & ls_certType
		set rs = DBQuery(ls_sql)
		'Insert new value
		ls_sql = "INSERT INTO employee_cert_numbers (employee_id, category_id, cert_number) " & _
		 "values (" & ls_empid & ", '" & ls_certType & "', '" & ls_CertNum & "')"
		set rs = DBQuery(ls_sql)
	Next
...
ElseIf trim(Request("InsertUpdate")) = "E" Then
	...
	ls_fax = Clean(trim(Request("fax")))
	ls_msngrid = Clean(trim(Request("msngrid")))
	if not isempty(ls_vmail) then
		set rs = DBQuery("Update Employee Set FirstName = '" & ls_fname & "', LastName = '" & ls_lname & "', Address = '" & ls_address & "', City = '" & ls_city & "', State = '" & ls_state & "', Zip = '" & ls_zip & "', HomePhone = '" & ls_hphone & "', SpouseName = '" & ls_spouse & "', PagerNumber = '" & ls_pager & "', PagerEmail = '" & ls_pager_email & "', CellPhone = '" & ls_Cell & "', Client_ID = " & ls_ClientID & ", ClientEmail = '" & ls_clientemail & "', Client_Phone = '" & ls_clientphone & "', Birthday = '" & ls_birthday & "', PersonalWebsite = '" & ls_personal_website & "', VoiceMail = '" & ls_vmail & "', Fax = '" & ls_fax & "', Msngrid = '" & ls_msngrid & "' where EmployeeID = " & ls_empid)
	else
		set rs = DBQuery("Update Employee Set FirstName = '" & ls_fname & "', LastName = '" & ls_lname & "', Address = '" & ls_address & "', City = '" & ls_city & "', State = '" & ls_state & "', Zip = '" & ls_zip & "', HomePhone = '" & ls_hphone & "', SpouseName = '" & ls_spouse & "', PagerNumber = '" & ls_pager & "', PagerEmail = '" & ls_pager_email & "', CellPhone = '" & ls_Cell & "', Client_ID = " & ls_ClientID & ", ClientEmail = '" & ls_clientemail & "', Client_Phone = '" & ls_clientphone & "', Birthday = '" & ls_birthday & "', PersonalWebsite = '" & ls_personal_website & "', Fax = '" & ls_fax & "', Msngrid = '" & ls_msngrid & "' where EmployeeID = " & ls_empid)
	end if
