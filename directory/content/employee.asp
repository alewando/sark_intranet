...
Developer:    GINGRICE
Date:         08/18/1998
Description:  Displays listing of consultants alphabetically
-->
...
'---------------------------
set rs = DBQuery("select employeeid, firstname, lastname, address, city, state, zip, homephone, spousename, unlistedpersonal from employee where DateTermination is null and Branch_ID = " & Application("DefaultBranchID") & " order by lastname")
...
maxcols = 3
while not rs.eof
	Response.Write("<TR VALIGN=TOP>" & NL)
	for col = 1 to maxcols          
		Response.Write("<TD><FONT SIZE=1 face='ms sans serif, arial, geneva'>")
		if not rs.eof then
			...
			Response.Write("<a href='details.asp?EmpId=" & rs("employeeid") & "'>")
			Response.Write(ls_name & "</a><br>" & ls_address & "<br>&nbsp;")
			rs.MoveNext
			end if          
