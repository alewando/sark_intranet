...
If Request.Form.Item("Title") <> "" then
	...
	ls_sql = "INSERT INTO News (Title, Employee_ID, CreationDate, Description) "_
		& "VALUES ('" & Title & "', " & Session("ID") & ", '" & Date() & "', 'Training')" 
...
'	rs.MoveLast
	...
	filename = Server.MapPath("/intranet/news/content/articles") & "\article" & trim(NewArticleNumber) & ".txt"
	...
	Else
		...
		Response.Write("<BR>Your training class has been submitted for review.<BR>")
		Response.Write("Once reviewed and approved it will be posted to the Training section.<BR>")
		Response.Write("Please contact your Account Manager if you have any questions.<BR>")
		Response.Write("<BR><a href='../../training/content/about_training.asp'>Return to Training</a></center>")	
...
Else
 ...
 <!--<INPUT type=button class=button value="Cancel" onClick="window.close()" id=button name=button>-->
 <INPUT type=button class=button value="Cancel" onClick="window.location='../../training/content/about_training.asp'" id=button name=button>
 </font>
 </form>
 </CENTER>
 <% else %>
  <CENTER>Guest users cannot submit training events</CENTER><BR>
 <% end if %>
