...
<font size = 2.5>This page contains information on training classes, including dates, times, instructor(s) and agenda.</font>
<p>

<b>Select a training class from the list...</b>
...
sql = "SELECT News.News_ID, News.Title,Employee.EmployeeID, News.CreationDate, Employee.FirstName,Employee.LastName " & _
		...
		"WHERE News.Description LIKE 'Training' AND News.Approved=1" & _
		"ORDER BY News.CreationDate DESC"
set rs = DBQuery(sql)
while not rs.eof
	response.write("<li>" & chr(34) & "<a href='training.asp?NewsID=" & rs("News_ID") & "' onMouseOver=" & chr(34) & "statmsg('View article.'); return true;" & chr(34) & " onMouseOut=" & chr(34) & "statmsg('')" & chr(34) & ">" & rs("Title") & "</a>" & chr(34) & "<br>")
