...
function createLayer(name, left, top, width, height, visible, content) {
  ...
  if (document.layers) {
    document.writeln('<layer name="' + name + '" id="' + name + '" left=' + left + ' top=' + top + ' width=' + width + ' height=' + height +  ' visibility=' + (visible ? '"show"' : '"hide"') + ' z-index=' + z + '>');
  ...
  if (document.all) {
    document.writeln('<div name="' + name + '" id="' + name + '" style="position:absolute; overflow:none; left:' + left + 'px; top:' + top + 'px; width:' + width + 'px; height:' + height + 'px;' + ' visibility:' + (visible ? 'visible;' : 'hidden;') + ' z-index:' + z + '">');
    document.writeln(content);
...
function createRelativeLayer(name, left, top, width, height, visible, content) {
  ...
  if (document.layers) {
    document.writeln('<layer name="' + name + '" id="' + name + '" left=' + left + ' top=' + top + ' width=' + width + ' height=' + height +  ' visibility=' + (visible ? '"show"' : '"hide"') + ' z-index=' + z + 'style="position:relative">');
    document.writeln(content);
    document.writeln('</layer>');
    layer = getLayer(name);
    layer.width = width;
  ...
  if (document.all) {
    document.writeln('<div name="' + name + '" id="' + name + '" style="position:relative; overflow:none; left:' + left + 'px; top:' + top + 'px; width:' + width + 'px; height:' + height + 'px;' + ' visibility:' + (visible ? 'visible;' : 'hidden;') + ' z-index:' + z + '">');
    document.writeln(content);
    document.writeln('</div>');
  }
...
function resize(name, intWidth, intHeight) {

  var layer = getLayer(name);
  if (document.layers)
    layer.resizeTo(intWidth,intHeight);
  else if (document.all) {
    layer.pixelWidth = intWidth;
	layer.pixelHeight = intHeight;
  }
  else
    return(null);
}
...
function getFormElement(frmname,name) {
  // Returns a handle to the named layer. 
  if (document.layers) { 
		return(document.forms[frmname].elements[name]);
  } else if (document.all) {
    layer = eval('document.all.' + name);
    return(layer);
  }
...
function getFormOrderElement(layerName,frmname,name) {
  // Returns a handle to the named layer.
  if (document.layers) {
		return(getElement(layerName).document.forms[frmname].elements[name]);
  } else if (document.all) {
    layer = eval('document.all.' + name);
    return(layer);
  }
...
function setFormOrderElement(layerName,frmname,name) {
  // Returns a handle to the named layer.
  if (document.layers) {
		return(getElement(layerName).document.forms[frmname].elements[name]);
  } else if (document.all) {
